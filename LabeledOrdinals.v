...
Inductive ord' : Type :=
  ...
  | Succ  : A -> ord' -> ord'
  | Limit : (nat -> ord') -> ord'.
(* TODO: Should Limit also be labeled? *)

Fixpoint pred_type (alpha : ord') : Type :=
  match alpha with
  | Zero         => False
  | Succ _ alpha => (unit + pred_type alpha) % type
...
Fixpoint pred (alpha : ord') : pred_type alpha -> ord' :=
  match alpha with
  | Zero         => !
  | Succ _ alpha => fun i => match i with
                             | inl tt => alpha
                             | inr t  => pred alpha t
                             end
...
Inductive ord'_le : ord' -> ord' -> Prop :=
  | Ord'_le_Zero  : forall alpha,
                      Zero <=' alpha
  | Ord'_le_Succ  : forall alpha v beta i,
                      alpha <=' pred beta i ->
                      Succ v alpha <=' beta
...
Lemma first_pred_after_succ_id :
  forall alpha v, alpha = pred (Succ v alpha) (inl (pred_type alpha) tt).
Proof.
trivial.
Qed.
...
Lemma ord'_le_not_succ_zero :
  forall alpha v, ~ Succ v alpha <=' Zero.
Proof.
intros alpha v H.
inversion_clear H.
destruct i.
...
Lemma ord'_le_not_succ_succ_one :
  forall alpha v w x, ~ Succ w (Succ v alpha) <=' Succ x Zero.
Proof.
intros alpha v w x H.
inversion_clear H.
destruct i.
destruct u.
apply (@ord'_le_not_succ_zero alpha v).
assumption.
assumption.
Qed.
...
Lemma ord'_le_zero_all :
  forall alpha beta,
    ...
    alpha <=' beta.
Proof.
induction alpha as [| alpha v _ | f IH]; intros beta H.
constructor.
inversion_clear H.
destruct i.
inversion_clear H.
...
Lemma ord'_le_pred_right :
  forall alpha beta (i : pred_type beta),
    ...
    alpha <=' beta.
Proof.
induction alpha as [| alpha v IH | f IH]; intros beta i H.
constructor.
apply Ord'_le_Succ with i.
inversion H as [| a w b j |].
apply IH with j.
assumption.
constructor.
intro n.
