(* Try yet a different pred_type for Limit *)
...
Fixpoint pred_type (alpha : ord') : Set :=
  ...
  | Limit f    => (nat + { n : nat & pred_type (f n) }) % type
  end.
...
constructor.
inversion_clear H.
apply Ord'_le_Succ with (inr nat (existT (fun n => pred_type (g n)) n i)).
assumption.
constructor.
intro m.
apply (IH m g n).
...
destruct i as [n | [n i]];
  exists (inl _ (S n));
  destruct (proj2 (gb n) (S n)) as [j H1];
  auto;
  apply Ord'_le_Succ with j;
