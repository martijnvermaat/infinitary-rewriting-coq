...
Lemma terms_bis_ind_cast :
  ...
  P 0 Vnil Vnil ->
  ( forall (t u : term) (n : nat) (v : terms n) (m : nat) (w : terms m) (Hmn : m = n) (w' := vector_cast w Hmn),
    term_bis t u -> terms_bis v w' -> P n v w' -> P (S n) (Vcons t v) (Vcons u w')
  ) ->
  forall (n : nat) (v : terms n) (m : nat) (Hmn : m = n) (w : terms m) (w' := vector_cast w Hmn),
...
assert (Hcast := vector_cast_elim w (refl_equal n)).
rewrite <- Hcast.
...
Inductive term_eq_up_to : nat -> term -> term -> Prop :=
  ...
  | teut_fun : forall n f (v w : terms (arity f)), 
...
with terms_eq_up_to : nat -> forall m : nat, vector term m -> vector term m -> Prop :=
  ...
  | teut_cons : forall n (t u : term), term_eq_up_to n t u -> 
...
Proof.
intros n v w.
rewrite <- (vector_cast_elim w (refl_equal n)).
apply terms_eq_up_to_0_cast.
Qed.

Lemma term_bis_to_term_eq : 
...
assert (H1 := fun n => (term_eq_fun_inv_vector (H (S n)) H0)).
