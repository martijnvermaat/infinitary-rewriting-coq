...
Record sequence : Type := {
  ...
  steps : forall alpha, (Succ alpha) <= length -> step;

  steps_fit :
    forall alpha (H : (Succ alpha) <= length),
      source (steps alpha H) [=] terms alpha (ord_le_succ_left alpha length H) /\
      target (steps alpha H) [=] terms (Succ alpha) H
  ...
  forall f (Hf : Limit f < length s) n,
    exists alpha,
      ...
      forall beta (Hb : beta < Limit f),
        alpha < beta ->
        term_eq_up_to n (terms s beta (ord_lt_trans beta (Limit f) (length s) Hb Hf))
                        (terms s (Limit f) Hf).
