...
# coq_makefile Context.v Finite_term.v Naturals.v Ordinals.v Rewriting.v Signature.v Substitution.v Term.v Term_equality.v TreeOrdinals.v Variables.v Vector.v Vector2.v prelims.v -noinstall -o makefile 
#
...
.PHONY: all opt byte archclean clean install depend html -noinstall
...
install:
	...
	(cd -noinstall; $(MAKE) INSTALLDEFAULTROOT=$(INSTALLDEFAULTROOT)/-noinstall install)
...
clean:
	...
	- rm -rf html
	(cd -noinstall ; $(MAKE) clean)

archclean:
	rm -f *.cmx *.o
	(cd -noinstall ; $(MAKE) archclean)
